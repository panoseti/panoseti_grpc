...
def read_json(f: Any) -> str | None:
    ...
    if c == b"":
        return None
    if c != b"{":
        raise Exception(f"read_json(): expected {{, got {c}")
    s = "{"
    last_nl = False
    while True:
        c = f.read(1)
...
def wr_to_unix_decimal(pkt_tai: Any, pkt_nsec: Any, tv_sec: Any) -> Decimal | int:
    ...
    d = (tv_sec - pkt_tai + 37) % 1024
    if d == 0:
        return tv_sec_d + pkt_nsec_d / nanosec_factor
    elif d == 1:
        return tv_sec_d - 1 + pkt_nsec_d / nanosec_factor
    elif d == 1023:
        return tv_sec_d + 1 + pkt_nsec_d / nanosec_factor
    else:
        return 0
...
def wr_to_unix_numpy(pkt_tai: Any, pkt_nsec: Any, tv_sec: Any) -> Any:
    ...
    d = (tv_sec - pkt_tai + 37) % 1024
    if d == 0:
        return tv_sec_n + pkt_nsec_n / np.longdouble(1e9)
    elif d == 1:
        return tv_sec_n - 1 + pkt_nsec_n / np.longdouble(1e9)
    elif d == 1023:
        return tv_sec_n + 1 + pkt_nsec_n / np.longdouble(1e9)
    else:
        return 0
