...
class UbloxControlClient:
    ...
    def capture_ublox(self, host: str, patterns: list[str] | None = None) -> Generator[dict[str, Any], None, None]:
...
class AioUbloxControlClient:
    ...
    async def capture_ublox(self, hosts: list[str], patterns: list[str] | None = None) -> AsyncIterator[dict[str, Any]]:
        ...
        tasks = [asyncio.create_task(_forward(s, h)) for s, h in zip(streams, hosts, strict=False)]
        finished = 0
