...
class DaqControlServicer(daq_control_pb2_grpc.DaqControlServicer):
    ...
    def _cleanup_dir(self, datadir: str | Path, run_dir: str, module_id: list[int]) -> None:
        import shutil
    ...
    async def StatusDaq(
        ...
        try:
            ...
            if dreq.get("check_run_dirs", False) and await anyio.Path(data_dir).exists():
                # Runs are directories ending in .pffd
                runs = []
                for d in os.listdir(data_dir):
                    if d.endswith(".pffd") and await anyio.Path(os.path.join(data_dir, d)).is_dir():
                        runs.append(d)
                res.run_dirs.extend(runs)
    ...
    async def CleanupData(
        ...
        if n > 0:
            msg = "Refusing to cleanup data while HASHPIPE is running. Call StopDaq first."
            self.logger.error(msg)
            return daq_control_pb2.CleanupDataResponse(success=False, message=msg)

        try:
            # Perform deletion
            await asyncio.to_thread(self._cleanup_dir, datadir, run_dir, module_id)
...
async def serve(port: int, logging_level: int = logging.INFO) -> None:
    ...
    await server.start()
    logging.info(f"Daq Control Server started on port {port}")
