...
async def test_lifecycle_serialization(servicer, tmp_path):
    ...
    request = daq_control_pb2.StartDaqRequest(
        data_dir=str(tmp_path), daq_ip_addr="127.0.0.1", bindhost="lo", run_dir="test.pffd", obs="test", module_id=[1]
    )
...
async def test_cleanup_data_acquires_lock(servicer, tmp_path):
    ...
    request = daq_control_pb2.CleanupDataRequest(data_dir=str(tmp_path), run_dir="test", module_id=[1])
