...
def test_unified_reflection_includes_all_services(start_unified_server):
    """The all-services server advertises all three services via reflection."""
    services = get_reflected_services("localhost", GRPC_PORT)
    assert any("Telemetry" in s for s in services), (
        f"Telemetry not in reflection: {services}"
    )
    assert any("DaqData" in s for s in services), (
        f"DaqData not in reflection: {services}"
    )
    assert any("DaqControl" in s for s in services), (
        f"DaqControl not in reflection: {services}"
    )


def test_unified_reflection_includes_reflection_service(start_unified_server):
    """The reflection service itself is included in the reflection response."""
    services = get_reflected_services("localhost", GRPC_PORT)
    assert any("reflection" in s.lower() for s in services), (
        f"Reflection service not advertised: {services}"
    )
...
def test_headnode_reflection_includes_only_telemetry(start_headnode_server):
    """Headnode profile advertises only the Telemetry service."""
    services = get_reflected_services("localhost", HEADNODE_PORT)
    assert any("Telemetry" in s for s in services), (
        f"Telemetry not in headnode reflection: {services}"
    )
    assert not any("DaqData" in s for s in services), (
        f"DaqData should not appear in headnode reflection: {services}"
    )
...
def test_daq_node_reflection_excludes_telemetry(start_daq_node_server):
...
def test_daq_node_reflection_includes_daq_services(start_daq_node_server):
    """DAQ node profile advertises DaqData and DaqControl via reflection."""
    services = get_reflected_services("localhost", DAQ_NODE_PORT)
    assert any("DaqData" in s for s in services), (
        f"DaqData not in daq_node reflection: {services}"
    )
    assert any("DaqControl" in s for s in services), (
        f"DaqControl not in daq_node reflection: {services}"
    )
