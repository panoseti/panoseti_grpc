...
def test_unknown_device_type_stays_in_sandbox(grpc_client, redis_client):
    ...
    assert poll_redis_key(redis_client, sandbox_key), \
        f"SANDBOX key {sandbox_key!r} not created"

    # Must NOT exist under any other prefix
    keys = redis_client.keys(f"*{device_id}*")
    for k in keys:
        assert k == sandbox_key, (
            f"Unknown device appeared outside SANDBOX: {k!r}"
        )
...
def test_extra_data_flattening_with_prefix(grpc_client, redis_client):
    ...
    assert poll_redis_field(redis_client, key, "extra_hdop"), \
        "extra_hdop not found in Redis hash"
...
def test_strict_schema_rejects_invalid_satellite_count(grpc_client):
    ...
    with pytest.raises(ValueError, match="Server rejected data"):
        grpc_client.log_strict("gnss", _unique("bad_gnss"), {
...
def test_strict_schema_rejects_out_of_range_coordinates(grpc_client):
    ...
    """
    with pytest.raises(ValueError, match="Server rejected data"):
        grpc_client.log_strict("gnss", _unique("bad_coords"), {
            "satellites": 8,
            "lat": 91.0,  # > 90, invalid
            "lon": 0.0,
            "fix_mode": "3D",
...
def test_large_payload_within_limit(grpc_client, redis_client):
    ...
    assert poll_redis_field(redis_client, key, "blob"), \
        f"'blob' field not found in {key!r} after large payload"

    stored = redis_client.hget(key, "blob")
    assert len(stored) == 100_000, (
        f"Expected stored blob length 100000, got {len(stored)}"
    )
...
def test_production_key_ttl_remains_permanent_after_multiple_writes(grpc_client, redis_client):
    ...
    assert ttl == -1, (
        f"Production key {key!r} acquired TTL={ttl} after writes "
        "(should always be -1 / no expiry)"
    )
...
def test_consecutive_field_overwrites_last_writer_wins(grpc_client, redis_client):
    ...
    assert float(final) == float(UPDATES - 1), (
        f"Expected counter={float(UPDATES - 1)}, got {final}"
    )
...
def test_two_device_types_do_not_share_namespace(grpc_client, redis_client):
    ...
    grpc_client.log_strict("gnss", device_id, {
        "satellites": 5, "lat": 10.0, "lon": 10.0, "fix_mode": "2D",
    })
    ...
    assert redis_client.hget(gnss_key, "temperature") is None, \
        "temperature field leaked into GNSS key"
    # GNSS-specific fields must NOT appear in flex key
    assert redis_client.hget(flex_key, "satellites") is None, \
        "satellites field leaked into flex key"
