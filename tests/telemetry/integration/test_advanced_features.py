...
def wait_for_worker_logs(redis_client, session_id, min_count=1, timeout=15):
    ...
    while time.time() - start < timeout:
        ...
        count = sum(1 for l in logs if session_id in l)
        if count >= min_count:
            return True
        time.sleep(0.5)
    return False
...
def test_distributed_session_switching(redis_client, start_grpc_server):
    ...
    # Wait for resume
    assert wait_for_worker_logs(redis_client, session_beta), \
        f"Workers did not resume for {session_beta} within timeout"
