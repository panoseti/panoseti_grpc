...
async def test_v2_data_flow(v2_server):
    ...
    sim_proc = await asyncio.create_subprocess_exec(
        ...
        "--socket-template", socket_template,
    ...
    try:
        ...
        if sim_proc.returncode is None:
            ...
            if stdout: print(f"Simulator STDOUT: {stdout.decode()}")
            if stderr: print(f"Simulator STDERR: {stderr.decode()}")
            
        if fwd_proc.returncode is None:
            fwd_proc.terminate()
            stdout, stderr = await fwd_proc.communicate()
            if stdout: print(f"Forwarder STDOUT: {stdout.decode()}")
            if stderr: print(f"Forwarder STDERR: {stderr.decode()}")
