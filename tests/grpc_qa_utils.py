...
class SuiteConfig(BaseModel):
    ...
    # For tasks that run as a raw shell command instead of docker exec
...
class Result:
    ...
    def __init__(
        self, name: str, code: int, elapsed: float, stats: dict[str, int] | None = None, stdout: str = ""
    ) -> None:
        self.name = name
        self.code = code
...
QA_TOML_PATH = GRPC_ROOT / "tests" / "qa.toml"
...
class TestRunner:
    def __init__(self, config_path: Path):
        ...
        self.project_prefix = self.cfg.settings.get("project_prefix", "pseti-grpc")

    async def run_suite(
        self, suite_name: str, jobs: int | None = None, target: str | None = None, extra_args: list[str] | None = None
    ) -> bool:
        ...
        project_name = suite.project_name or f"{self.project_prefix}-{suite.name}"
    ...
    async def build_all(self):
        ...
        for suite in self.cfg.suites.values():
            if suite.requires_docker and suite.compose_file and suite.compose_file not in processed_files:
                project_name = f"{self.project_prefix}-build"
                cmd = f"{self.container_tool} compose -f {GRPC_ROOT}/tests/{suite.compose_file} build"
    ...
    async def _setup_docker(self, suite: SuiteConfig, project_name: str):
        ...
        build_flag = " --build" if not self.no_build else ""
        cmd = f"{self.container_tool} compose -f {GRPC_ROOT}/tests/{compose_file} {profile_str} up -d{build_flag} --remove-orphans"
        ...
        if suite.pre_run:
            ...
            pre_cmd = f"{self.container_tool} compose -f {GRPC_ROOT}/tests/{compose_file} {profile_str} exec -T {suite.service} /bin/sh -c '{suite.pre_run}'"
    ...
    async def _teardown_docker(self, suite: SuiteConfig, project_name: str, quiet: bool = False):
        ...
        cmd = (
            f"{self.container_tool} compose -f {GRPC_ROOT}/tests/{compose_file} {profile_str} down -v --remove-orphans"
        )
        await self._run_cmd(cmd, env={"COMPOSE_PROJECT_NAME": project_name}, quiet=quiet)

    async def _run_test_suite(
        self, suite: SuiteConfig, project_name: str, jobs: int | None, extra_args: list[str] | None
    ) -> list[Result]:
        ...
        if suite.raw_command:
            # Run raw command from host
            lock = asyncio.Lock()
            res = await self._stream(f"test.{suite.name}", suite.raw_command, lock)
            return [res]
        ...
        if compose_file:
            cmd = f"{self.container_tool} compose -f {GRPC_ROOT}/tests/{compose_file} {profile_str} exec -T {env_str} {suite.service} {pytest_cmd}"
        else:
            # Run locally if no docker required
            cmd = pytest_cmd
    ...
    async def _run_lint_suite(
        self, suite: SuiteConfig, project_name: str, target: str | None, extra_args: list[str] | None
    ) -> list[Result]:
        ...
        async def run_task(name: str, task_cmd: str):
            if compose_file:
                cmd = f"{self.container_tool} compose -f {GRPC_ROOT}/tests/{compose_file} {profile_str} exec -T {suite.service} {task_cmd} {extra_str}"
            else:
                cmd = f"{task_cmd} {extra_str}"
            tag_text = f"[{name}] "
            return await self._stream(
                f"lint.{name}", cmd, lock, tag=tag_text, env={"COMPOSE_PROJECT_NAME": project_name}
            )
    ...
    async def _run_cmd(
        self, cmd: str, env: dict[str, str] | None = None, quiet: bool = False, capture: bool = False
    ) -> Result:
        ...
        proc = await asyncio.create_subprocess_shell(
            cmd, stdout=stdout_dest, stderr=asyncio.subprocess.DEVNULL if quiet else None, env=full_env
        )
    ...
    async def _stream(
        self, name: str, cmd: str, lock: asyncio.Lock, tag: str = "", env: dict[str, str] | None = None
    ) -> Result:
        start = time.monotonic()
        full_env = os.environ.copy()
        if env:
        ...
        async for raw in proc.stdout:
            ...
            if not has_json_metrics:
                is_result = (
                    plain_line.startswith("[gw")
                    and any(kw in upper_line for kw in [" PASSED", " FAILED", " SKIPPED", " ERROR", " XFAIL", " XPASS"])
                ) or (
                    "::" in plain_line
                    and any(
                        upper_line.endswith(kw) or f"{kw} [" in upper_line
                        for kw in ["PASSED", "FAILED", "SKIPPED", "ERROR", "XFAIL", "XPASS"]
                    )
                )
                if is_result:
