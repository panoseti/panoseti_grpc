...
_See [`docs/quickstart.md`](docs/quickstart.md) for build, test, lint, and run commands._
...
- **`OSError: [Errno 5] Input/output error` from `configure_logger`**: Docker's overlay2 filesystem returns EIO when `RotatingFileHandler.stream.close()` is called on a file descriptor pointing to a file deleted by `CleanupData`'s `rmtree`. The fix (in `telemetry/logger.py:configure_logger`) wraps `h.close()` in `try/except OSError: pass` and always calls `logger.removeHandler(h)` regardless. Without this, the second `StartDaq` after a `CleanupData` raises `INTERNAL` from the gRPC error handler.
